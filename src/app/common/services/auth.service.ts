...
export class AuthService {
    ...
    renewToken(): Observable<LoginCredentials> {
        return this.httpClient.post<LoginCredentials>('/api/auth/refresh-tokens', {
            ...
            catchError((error) => {
                this.jwtResult = undefined;
                return throwError(() => new Error('Problem found when refreshing token. Please sign in again.'));
            }),
            shareReplay(1)
        );
    }
